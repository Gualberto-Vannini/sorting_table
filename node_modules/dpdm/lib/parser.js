...
function parseDependencyTree(entries, options) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        yield Promise.all(entries.map((entry) => glob_1.default.glob(entry).then((matches) => Promise.all(matches.map((filename) => parseTreeRecursive(currentDirectory, path_1.default.join(currentDirectory, filename), fullOptions, output, resolve))))));
